...
def get_high_expression_in_safety_tissues(expression_index):
	...
	# Parses the OT expression index to flag high expression (level=3) in one of the organs marked as important for preliminary safety studies

	# Notes:
	# i) jejunum is not available,replaced by small intestine,
	# ii) eye is under sense organ
	# iii) thymus hasn't been assigned to an organ/tissue (probably immune organ? alt. immune system/lymphoid system?),
	...
	with open(expression_index ) as expressionfile:
		...
		for line in expressionfile:
			...
			for tissue in tissues:
				...
				if any(org in safety_organs for org in organs): # only look at expression if organ in my list
					if tissue['rna']['level']==3: # check RNA expression
					...
					elif any(cell['level']==3 for cell in tissue['protein']['cell_type']): # check protein expression
						safety_express.setdefault(ensembl_id, {}).setdefault('Bucket_T1:high_expression_in_safety_organs',set()).update([o for o in organs if o in safety_organs])
